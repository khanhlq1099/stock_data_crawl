class SSI_API_CONFIG:
    CONSUMER_ID = "a7a664018a9e4114a1a5812838a91e6b"
    CONSUMER_SECRET = "468fc69fe1654ff6b3f46937f954b010"
    PRIVATE_KEY = ""

    DEFAULT_REQUEST_API_DELAY_TIME = 5 # s
    DEFAULT_PAGE_SIZE_1 = 1
    DEFAULT_PAGE_SIZE_10 = 10
    DEFAULT_PAGE_SIZE_100 = 100
    DEFAULT_PAGE_SIZE_1000 = 1000

class DB_CONFIG:
    CONNECTION_STRING="DRIVER={ODBC Driver 18 for SQL Server};SERVER=10.10.10.20,1436;DATABASE=dw_stock;UID=etl;PWD=KPIM@#2022;Trusted_Connection=yes;TrustServerCertificate=Yes;"